...
public class AuthController extends HttpServlet implements PropLoader
{
     ...
     *
     * @param authRequest
     *         the request to the oauth2/token url in cognito
     *
    ...
    private String validate(TokenResponse tokenResponse) throws IOException
    {
        ...
        // TODO the following is "happy path", what if the exceptions are
        //  caught?
        // Create a public key
        ...
        // Verify ISS field of the token to make sure it's from the Cognito
        // source
        String iss =
                String.format("https://cognito-idp.%s.amazonaws.com/%s", REGION,
                              POOL_ID);

        JWTVerifier verifier = JWT.require(algorithm).withIssuer(iss)
                                  .withClaim("token_use",
                                             "id") // make sure you're
                                  // verifying id token
                                  .build();
        ...
        String     birthdate = jwt.getClaim("cognito:Birthdate").asString();
     ...
     *
     * @param authCode
     *         auth code received from Cognito as part of the login process
     *
    ...
    private HttpRequest buildAuthRequest(String authCode)
    {
        ...
        String form = parameters.keySet().stream().map(key -> key + "=" +
                                                              URLEncoder.encode(
                                                                      parameters.get(
                                                                              key),
    ...
    private void loadKey()
    {
        ...
        {
            logger.error("Cannot load json..." + ioException.getMessage(),
                         ioException);
        }
        catch (Exception e)
        {
